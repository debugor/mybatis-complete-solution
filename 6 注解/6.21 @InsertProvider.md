`@InsertProvider` 是 MyBatis 提供的一个注解，用于动态生成 SQL 插入语句的提供者方法。类似于 `@SelectProvider`，它允许你在注解中指定一个类，这个类中包含了动态生成 SQL 插入语句的逻辑。通过 `@InsertProvider` 注解，你可以更灵活地构建动态插入语句，根据不同的条件生成不同的 SQL。
